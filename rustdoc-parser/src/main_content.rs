use crate::{
    helpers::{Fragment, HtmlElement},
    sections::{parse_auto_traits, parse_implementations, parse_required_associated_types, parse_variants},
    traits::Body,
};
        ...
        MainContent(
            sections
                ...
                .map(|section| {
                    ...
                    let name =
                        String::from(HtmlElement::build(&mut split[0]).content[0].raw_content());
                    // LATERZ
                    ...
                    let content = match name.as_str() {
                        ...
                        "Required Associated Types" => parse_required_associated_types(split[1]),
...
pub struct Section {
    pub name: Fragment,
    pub content: Vec<Box<dyn Body>>,
}

impl Section {
    pub fn build(name: String, content: Vec<Box<dyn Body>>) -> Self {
        Section {
            name: Fragment::Bold(Vec::from([Fragment::Raw(name)])),
            content,
        }
